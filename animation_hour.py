...
def karten_visual(j, minimum, maximum):
    ...
    draw.text((10,10), DATUM + '   ' + str(UHRZEIT) + ' Uhr', (0,0,0), font=font)
    ...
    card.save('Output/GIF2/bild_00' + str(j) + '.png')
...
if __name__=="__main__":
    ...
    frames = []
    py_path = os.getcwd()
    cards = sorted_alphanumeric(os.listdir(py_path + 'Output/GIF2'))
    i = 0
    for card in cards:
        card = Image.open(py_path + 'Output/GIF2/' + card)
        #Konvertierung RGB
        background= Image.new('RGB',card.size,(255,255,255))
        background.paste(card,mask=card.split()[3])
        background.save(f"Output/cardrgb_forgif2/frame{i}.png", quality=100)
        frames.append(background)
        i += 1
    
    frames[0].save("Output/Messverlauf_Stunde.gif", save_all=True, append_images=frames[1:], duration=1000, loop=0)
